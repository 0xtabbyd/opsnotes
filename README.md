...
OpsNotes は **自分のPCの `127.0.0.1`（localhost）で単独利用する**ことを前提としたツールです。認証機構はありません。

### v0.5 で修正した脆弱性
v0.4 で導入した機能に含まれていた以下の問題を修正しました。

- **CORS の全許可を撤去**: 閲覧中の任意の外部サイトが、あなたのブラウザ経由で OpsNotes の全データを読み書きできる状態でした。同一オリジンで完結するため CORS 設定自体を削除しています。
- **Markdown プレビューの XSS 対策**: メモ本文の HTML を無検査で描画していたため、細工したメモを開くとスクリプトが実行されました。DOMPurify で描画前に無害化します（構成図の SVG も同様）。
- **画像アップロードの上限（10MB）と SVG 除外**: サイズ無制限だった点と、スクリプトを含む SVG が同一オリジンで配信されていた点を塞ぎました。
- **draw.io 連携の送信元検証**: iframe からの postMessage の origin を検証します。
- **画像をDBに格納しデータ保全へ統合**: 画像の実体を `data/uploads/` のファイルではなく DB に保存するようにしました。これにより**バックアップ・JSONエクスポート・ロールバックが画像も自動的に含みます**。旧バージョンの `data/uploads/` 内の画像は初回起動時にDBへ自動移行します。どのメモからも参照されなくなった画像は、メモ削除時に自動回収されます（アップロード直後の画像は1時間の猶予で保護）。

### 既知の制限（未対応）
- 認証が無いため、`OPSNOTES_HOST` を `0.0.0.0` 等にした共有利用には向きません（下記「設定」を参照）。
- 複数同時アクセス時、同期I/Oを `async` で実行しているためリクエストが直列化します（単独利用では問題ありません）。
...
> ⚠️ **セキュリティ上の注意**: OpsNotes には認証機構がありません。`OPSNOTES_HOST` を `127.0.0.1` 以外（`0.0.0.0` 等）にすると、そのアドレスに到達できる**ネットワーク上の全員が全データを閲覧・改変できます**。信頼できないネットワークでは既定の `127.0.0.1`（自分のPCからのみアクセス可能）のまま使ってください。`0.0.0.0` で起動すると警告ログが出ます。
