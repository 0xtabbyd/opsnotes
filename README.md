...
### 3. draw.io 連携・インフラ構成図作図 (draw.io Embed Integration - v0.4新機能)
- 🌐 **業界標準 draw.io (diagrams.net) の完全統合**
  - メモ作成画面の「作図・挿入」ボタンから、フル機能のdraw.ioエディタ（日本語UI）がシームレスに展開。
  - Cisco、AWS、Azure、GCP、Kubernetes、フローチャート、UMLなど数万種類に及ぶ豊富なアイコン・シェイプライブラリを自在に利用可能。
- 🖼️ **MarkdownへのベクターSVG埋め込み & 即座の再編集**
  - 作図内容は完全なXMLモデルと最適化されたベクターSVGとしてMarkdown（```drawio）に自動保存。
  - ライブプレビューやPDF印刷時にも外部通信を待たずに高品質なベクターグラフィックスとして即座に描画。
  - 挿入済みの構成図は右上の「draw.ioで編集」ボタンからいつでもワンクリックで編集状態を復元・更新可能。
- 📄 **改ページ制御 & PDF出力最適化**
  - ツールバーの「改ページ」ボタンにより、PDF出力時に指定箇所で確実に改ページ。
  - 構成図も清潔な白背景で自動整形され、印刷・PDFプレビューが美麗に出力。
- 🔄 **旧キャンバス形式の後方互換性**
  - v0.2/v0.3で作成された従来の `opsnotes-canvas` 形式のメモもそのままプレビュー表示可能。
